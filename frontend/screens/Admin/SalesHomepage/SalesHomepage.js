...
const TOKEN= "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2MDA3ZjRmM2VmOTRjMTAwMjQ4ODI1N2QiLCJpYXQiOjE2NzExMjgwMjF9.wif-XB3NuXqAFdZIj6os7Hfb2EU_Rm2_0cDahrsEXKg"
  ...
  const Item = (props) => (
    ...
    >
          ...
          >
            ...
            <Text style={styles.title}>Payment Status: {props.order.payment_status}</Text>
            <Text style={styles.title}>No of items: {props.order.products.numberOfItem}</Text>
            <Text style={styles.title}>Cost: {props.order.products.cost}</Text>
            <Text style={styles.title}>Amount recieved: {props.order.orderReceivedAmount}</Text>
            <Text style={styles.title}>Amount remaining: {props.order.orderRemainingAmount}</Text>
          </View>
    </TouchableOpacity>
  );
...
//   console.log(str[0].includes(''))
 ...
 const getAllProducts= () => {
  ...
  //  axios.get(API , { headers: {"Authorization" : `Bearer ${TOKEN}`} })
   ...
   setData(res.data)
   setLiveOrders(res.data.filter(i => checklLiveOrder(i.ordered) === true))
   setPastOrders(res.data.filter(i => checklLiveOrder(i.ordered) === false))
   setLoading(false)
   }).catch((error) => {
   console.log(error)
   setLoading(false)
